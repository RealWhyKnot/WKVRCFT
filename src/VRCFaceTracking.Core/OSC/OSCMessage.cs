...
public class OscMessage
{
    ...
    public object? Value
    {
        ...
        set
        {
            if (_valueSetter != null && value != null)
                _valueSetter(value);
        }
    }
    ...
    public OscMessage(string address, Type type)
    {
        ...
        if (oscType != default)
        {
            ...
            var oscValue = new OscValue { Type = oscType.oscType };
            _valueSetter = value =>
            {
    ...
    public static OscMessage? TryParseOsc(byte[] bytes, int len, ref int messageIndex)
    {
        ...
        return msg._metaPtr == IntPtr.Zero ? null : msg;
    }
    ...
    public async Task<int> Encode(byte[] buffer, CancellationToken ct) =>
        await Task.Run(() => fti_osc.create_osc_message(buffer, ref _meta), ct);

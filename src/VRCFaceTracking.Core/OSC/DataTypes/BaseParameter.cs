...
public class BaseParam<T> : Parameter where T : struct
{
    ...
    public bool Relevant
    {
        ...
        protected set
        {
            ...
            if (value) UnifiedTracking.OnUnifiedDataUpdated += Process;
            else UnifiedTracking.OnUnifiedDataUpdated = (UnifiedTracking.OnUnifiedDataUpdated - Process)!;
        }
    }
    ...
    public override Parameter[] ResetParam(IParameterDefinition[] newParams)
    {
        ...
        var compatibleParam = newParams.FirstOrDefault(param =>
            _regex.IsMatch(param.Address) && param.Type == typeof(T));
    ...
    ~BaseParam()
    {
        UnifiedTracking.OnUnifiedDataUpdated = (UnifiedTracking.OnUnifiedDataUpdated - Process)!;
    }
}

...
public class Calibration : TrackingMutation
{
    ...
    public static float cDelta = 0.1f;
    ...
    public class CalibrationParameter
    {
        ...
        private float Normalize(float currentValue) => currentValue / max;
    ...
    public class CalibrationData
    {
        ...
        {
            for (int i = 0; i < Shapes.Length; i++)
                Shapes[i].UpdateCalibration(currentValue: values[i], continuous: continuous, logger: logger, dT: ms / 1000f);
        }
    ...
    public override void MutateData(ref UnifiedTrackingData data)
    {
        ...
        {
            calData.Shapes[i].UpdateCalibration(
                currentValue: data.Shapes[i].Weight,

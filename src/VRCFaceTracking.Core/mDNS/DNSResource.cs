...
public class DnsResource : DnsQuestion
{
    ...
    public DnsResource(IDnsSerializer data, List<string> names)
        : base(names, _typeMap.First(x => x.Value == data.GetType()).Key, 1)
    {
        Data = data;
    ...
    public override byte[] Serialize()
    {
        ...
        var data = Data?.Serialize() ?? Array.Empty<byte>();

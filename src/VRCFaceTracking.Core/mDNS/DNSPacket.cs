...
public class DnsPacket
{
    ...
    public DnsPacket(BigReader stream)
    {
        ...
        for (var i = 0; i < questions.Length;   i++) questions[i]   = new DnsQuestion(stream);
        for (var i = 0; i < answers.Length;     i++) answers[i]     = new DnsResource(stream);
        for (var i = 0; i < authorities.Length; i++) authorities[i] = new DnsResource(stream);
        for (var i = 0; i < additionals.Length; i++) additionals[i] = new DnsResource(stream);
    }
    ...
    public byte[] Serialize()
    {
        ...
        foreach (var question   in questions)   bytes.AddRange(question.Serialize());
        foreach (var answer     in answers)     bytes.AddRange(answer.Serialize());
        foreach (var authority  in authorities) bytes.AddRange(authority.Serialize());
        foreach (var additional in additionals) bytes.AddRange(additional.Serialize());
